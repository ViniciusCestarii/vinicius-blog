...
updatedAt: "2024-10-14T13:34:07.635Z"
status: "draft"
tags: 
- "jwt"
- "security"
...
- Custom claims like user role, permissions, etc.
...
The client stores the token, usually in <ExternalLink href="https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage">`localStorage`</ExternalLink> or <ExternalLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Cookies">`cookies`</ExternalLink>, and sends it with every request to the server.
The server verifies the token’s authenticity using the key it was signed with and grants access to protected resources if the token is valid.
